...
```bash
curl "https://api.aisa.one/v1/models/gemini-2.0-flash:generateContent?key=$API_KEY" \
...
curl "https://api.aisa.one/v1beta/models/gemini-2.0-flash:generateContent?key=$API_KEY" \
    -H 'Content-Type: application/json' \
    -X POST \
...
```bash
curl "https://api.aisa.one/v1beta/models/gemini-2.0-flash:streamGenerateContent?alt=sse&key=$API_KEY" \
    -H 'Content-Type: application/json' \
    --no-buffer \
    -d '{
      "contents": [{
...
```
POST https://api.aisa.one/v1beta/{model=models/*}:generateContent
```

#### Stream Generate Content

```
POST https://api.aisa.one/v1beta/{model=models/*}:streamGenerateContent
```

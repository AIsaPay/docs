...
| input\_reference | file | No | Input image file (used for Image-to-Video) |
...
```bash
curl http://api.aisa.one/v1/videos \
...
| progress | integer | Generation progress percentage |
| expires\_at | integer | Resource expiration timestamp |
...
| Parameter | Type | Required | Description |
|------|------|------|------|
| video\_id | string | Yes | Identifier of the video task to retrieve |
...
| quality | string | Video quality |
| remixed\_from\_video\_id | string | Identifier of the source video if this video is a remix |
...
async function generateAliVideo() ));
  ...
  const response = await fetch('http://api.aisa.one/v1/videos', ,
    body: formData
  });

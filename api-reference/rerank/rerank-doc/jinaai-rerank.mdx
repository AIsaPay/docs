...
"Standard Format"
    In AIsa, Jina AI's rerank format is adopted as the standard format. All other vendors' (such as Xinference, Cohere, etc.) rerank responses will be formatted to Jina AI's format to provide a unified development experience.
...
```bash
curl https://your-aisa-server-address/v1/rerank \
...
Where `$AISA_API_KEY` is your API key.

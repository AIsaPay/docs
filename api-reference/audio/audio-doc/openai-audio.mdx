...
```bash
curl https://api.aisa.one/v1/audio/speech \
...
```bash
curl https://api.aisa.one/v1/audio/transcriptions \
  -H "Authorization: Bearer $API_KEY" \
...
```bash
curl https://api.aisa.one/v1/audio/translations \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: multipart/form-data" \
...
- Required: No
- Default: json
- Optional values: json, text, srt, verbose\_json, vtt
